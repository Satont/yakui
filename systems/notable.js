...
class Notable {
  ...
  async init() {
    this.settings = (await global.db.select('*').from('settings').where('system', 'notable'))[0].data
  }
  async np() {
  ...
  async addacc(param) {
    try {
      ...
      let message = this.settings.accountadded.replace('$id', param)
      return message
    } catch (e) {
      let message = e.response.data.description
  ...
  async delacc(param) {
    try {
      ...
      let message = this.settings.accountdeleted.replace('$id', param)
      return message
    } catch (e) {
      let message = e.response.data.description
      message = message.replace("Account", this.settings.account)
  ...
  async listacc() {
    try {
      let request = await axios(`http://aibt.ga/api/listacc?channel=${process.env.TWITCH_CHANNEL.toLocaleLowerCase()}&token=${this.settings.token}`);
  ...
  async medal() {
    try {
      let request = await axios(`http://aibt.ga/api/medal?channel=${process.env.TWITCH_CHANNEL.toLocaleLowerCase()}&token=${this.settings.token}`);
      let response = await request.data
  ...
  async sockets() {
    ...
    io.on('connection', function (socket) {
      socket.on('settings.notable', async (data, cb) => {
        let query = await global.db('settings').select('*').where('system', 'notable')
        cb(null, query[0])
      })
