...
<div>
    ...
    <div class="input-group-append">
        <button class="btn btn-info" type="button" id="popover" data-toggle="popover" title="Where to get token?" data-content="Settings -> API settings -> API tokens -> Your Socket API Token">?</button>
    </div>
  </div>
</div>
</template>
...
export default {
  ...
  mounted() {
    let self = this;
    this.$socket.emit('settings.streamlabs', null, (err, settings) => {
  ...
  methods: {
    save() {
      ...
      this.$socket.emit('update.settings.streamlabs', data)
    }
  },
};
</script>
