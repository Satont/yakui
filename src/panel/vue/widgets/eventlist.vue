<template>
  <b-list-group>
    ...
    <b-list-group-item v-for="event of events.filter((e) => avaliableEvents.includes(e.name))" v-bind:key="event.id">
      ...
      <div>
        <span v-if="event.data.message" style="font-size: 14px;">{{ event.data.message }}</span
        ><br v-if="event.data.message" />
        ...
        <span v-if="event.name === 'hosted' || event.name === 'raided'"> {{ event.name }} with {{ event.data.viewers }} viewers </span>
      </div>
    </b-list-group-item>
  </b-list-group>
</template>
...
import { EventList } from '@prisma/client';
...
export default class Events extends Vue {
  ...
  humanize(val) {
    return humanizeDuration(Date.now() - val, {
      units: ['mo', 'd', 'h', 'm', 's'],
      round: true,
      language: this.$store.state.metaData.lang,
    });
  }
