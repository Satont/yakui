  ...
  <div class="dashboard">
      ...
      >
        <b-card style="color:#000" class="h-100" body-class="p-0 h-100" header-class="p-1">
          <template v-slot:header>
            <span class="title">{{ item.name | capitalize }}</span>
            <b-btn variant="danger" size="sm" class="float-right" @click="delWidget(item.id)">Delete</b-btn>
            <i class="fas fa-info-circle" v-b-tooltip.hover :title="descriptions[item.name]" v-if="descriptions[item.name]"></i>
          </template>
    ...
    <b-modal id="widgets" header-text-variant="dark" body-text-variant="dark" title="Choose widget" scrollable size="sm">
      <b-list-group>
        ...
        <b-list-group-item href="#" @click="addWidget(widget)" v-for="widget in availableWidgets" :key="widget">{{
          widget | capitalize
        }}</b-list-group-item>
      </b-list-group>
    </b-modal>
...
export default class Interface extends Vue {
  ...
  async getWidgets() {
    ...
    const widgets: Widgets[] = request.data;
