        ...
        <grid-item v-for="item in widgets" :x="item.x" :y="item.y" :w="item.w" :h="item.h" :i="item.i" :key="item.i" @resized="saveWidget" @moved="saveWidget">
          <b-card style="color:#000" class="h-100" body-class="p-0 h-100" header-class="p-1">
            <template v-slot:header>
              <span class="title">{{ item.name | capitalize }}</span> <b-btn variant="danger" size="sm" class="float-right" @click="delWidget(item.id)">Delete</b-btn> <i class="fas fa-info-circle" v-b-tooltip.hover :title="descriptions[item.name]" v-if="descriptions[item.name]"></i>
            </template>
